...
from fractalstat.fractalstat_entity import BitChain, Coordinates, Coordinates
from fractalstat.fractalstat_entity import FractalStatCoordinates, compute_adjacency_score
...
def generate_random_bitchain(seed: Optional[int] = None) -> BitChain:
    ...
    if use_ultra_coarse:
        ...
        luminosity_val = round(secure_random.uniform(0, 100), 0)  # Integers 0-100 (101 values)
    else:
    ...
    adjacency_score = min(100.0, len(adjacency_ids) * 20.0)
...
class EXP03_CoordinateEntropy:
    ...
    def compute_individual_contribution(self, dimension_name: str, bitchains: List) -> float:
        ...
        if dimension_name == "realm":
            max_values = len(Realm)  # 8 realm enum values
        elif dimension_name == "horizon":
            max_values = len(Horizon)  # 6 horizon enum values
        elif dimension_name == "polarity":
            max_values = len(Polarity)  # 12 polarity enum values
        elif dimension_name == "alignment":
            max_values = len(Alignment)  # 12 alignment enum values
