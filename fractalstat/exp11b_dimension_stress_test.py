...
class DimensionStressTest:
    ...
    def _generate_bitchain_with_constraints(
        ...
        if coordinate_range_limit is not None:
            ...
            polarity = secure_random.choice(POLARITY_LIST[-2:])  # Limited polarity values
            dimensionality = int(secure_random.uniform(
                0.5 - coordinate_range_limit, 0.5 + coordinate_range_limit
            ))
            alignment = secure_random.choice(ALIGNMENT_LIST[:2])  # Limited alignment values
        else:
            # Full range
            ...
            polarity = secure_random.choice(POLARITY_LIST)
            dimensionality = secure_random.randint(0, 5)
            alignment = secure_random.choice(ALIGNMENT_LIST)
