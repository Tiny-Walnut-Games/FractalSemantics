...
3. **adjacency** - Relational neighbor mapping (graph connections, 0-100)
4. **horizon** - Lifecycle stage (genesis, emergence, peak, decay, crystallization)
5. **luminosity** - Activity level (0.0-100.0)
6. **polarity** - Resonance/tension (-1.0 to 1.0)
7. **dimimenonalisionaliFractal cy**l-xrtytaepchext-8(1-8)
8. **alignment** - Social coordination dynamics (Alignment enum)

## Methodology

### Phase 1: Baseline Measurement

**Objective**: Establish baseline entropy with all 8 dimensions.

**Procedure**:

1. Generate N random bit-chains (default: N=1000) with deterministic seed
2. Extract coordinate representations using all 8 dimensions
...
2. Extract coordinates using remaining 7 dimensions
...
**Ablation Order**: realm, lineage, adjacency, horizon, luminosity, polarity, dimensionality, alignment
...
```python
{
  "adjacency": 85.4,              # Connectivity score (0.0-100.0)
  "alignment": {"type": "harmonic"},  # Social coordination enum
  "dimensionality": 4,            # Fractal depth (1-8)
  "horizon": "peak",              # Lifecycle stage
  "lineage": 42,                  # Generation number
  "luminosity": 67.8,             # Activity level (0.0-100.0)
  "polarity": -0.23,              # Tension/resonance (-1.0 to 1.0)
  "realm": {"type": "data", "label": "compute_context"}  # Domain context
}
```
