...
class BobStressTester:
    ...
    async def single_query(self, query_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.api_base_url}/query", params=params, timeout=30
            )
    ...
    async def query_worker(self, worker_id: int, duration_seconds: int):
        ...
        while time.time() < end_time:
            ...
            if queries_executed % 10 == 0:
                avg_time = (
                    statistics.mean(self.query_times[-10:]) if self.query_times else 0.1
                )
                target_interval = 1.0 / (
                    self.config.QUERIES_PER_SECOND_TARGET
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        bob_alert_rate = (
            (self.bob_verdicts["VERIFIED"] + self.bob_verdicts["QUARANTINED"])
            / total_bob_decisions
            if total_bob_decisions > 0
            else 0
        )
        bob_quarantine_rate = (
            self.bob_verdicts["QUARANTINED"] / total_bob_decisions
            if total_bob_decisions > 0
            else 0
        )
        ...
        report = {
            ...
            "detailed_results": self.results[
                -100:
            ],  # Last 100 results for detailed analysis
        }
        ...
        report_file = (
            ...
            / "results"
            / f"bob_stress_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    parser.add_argument(
        "--duration", "-d", type=int, default=30, help="Test duration in minutes"
    )
    parser.add_argument(
        "--qps", "-q", type=int, default=10, help="Target queries per second"
    )
    parser.add_argument(
        "--concurrent", "-c", type=int, default=50, help="Maximum concurrent queries"
    )
    parser.add_argument(
        "--api-url", "-u", default="http://localhost:8000", help="API base URL"
    )
