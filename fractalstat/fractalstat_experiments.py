...
def run_single_experiment(module_name: str, display_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Always run experiments as subprocess to prevent sys.exit() from killing orchestrator
        try:
            result = subprocess.run([
                sys.executable, "-m", f"fractalstat.{module_name}"
            ], capture_output=True, text=True, encoding='utf-8', cwd=Path(__file__).parent.parent, check=False)
