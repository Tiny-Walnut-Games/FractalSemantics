"""
EXP-03: Coordinate Space Entropy Test - Experiment Module

This module implements the core experiment logic for quantifying the information-theoretic
entropy contribution of each FractalStat dimension to the coordinate space. The experiment
measures how well each dimension contributes to semantic disambiguation through ablation
studies and comprehensive entropy analysis.

Core Scientific Methodology:
1. Generate baseline coordinate space with all 8 dimensions
2. Perform ablation studies by removing each dimension individually
3. Measure Shannon entropy at coordinate level (pre-hash)
4. Calculate semantic disambiguation power and expressiveness contributions
5. Determine critical dimensions and minimal necessary set

Key Information-Theoretic Insights:
- Shannon entropy measures information content of coordinate space
- Coordinate-level measurement captures semantic structure before hashing
- Ablation testing quantifies each dimension's contribution to information content
...
class EntropyConfig:
    """
    Configuration for coordinate entropy testing parameters.
    
    This class defines the dimensions and parameters used for entropy
    analysis across different dimensional configurations.
    """
    
    # The 8 FractalStat dimensions
    ...
    # Expressiveness contribution weights for composite scoring
    EXPRESSIVENESS_WEIGHTS = {
        'individual': 0.35,      # Individual contribution vs theoretical maximum
        'relative': 0.35,        # Relative contribution via marginal gains
        'complementary': 0.20,   # Complementary contribution (unique information)
        'ablation': 0.10         # Legacy ablation-based contribution
    }
    
    # Thresholds for critical dimension assessment
    CRITICALITY_THRESHOLDS = {
        'expressiveness': 5.0,   # Minimum expressiveness contribution percentage
        'entropy_reduction': 0.0 # Minimum entropy reduction percentage
    }


def generate_random_bitchain(seed: Optional[int] = None) -> BitChain:
    """
    Generate a random bit-chain for entropy testing and validation experiments.

    This function creates synthetic bit-chains with randomized but valid FractalStat
    coordinates. It's used extensively in entropy validation experiments to test
    coordinate space information content and semantic disambiguation.

    When a seed is provided, the function generates deterministic "random" data,
    which is essential for:
    - Reproducible entropy measurements
    ...
    Example:
        # Deterministic generation for reproducible entropy testing
...
class EXP03_CoordinateEntropy:
    ...
    def compute_individual_contribution(self, dimension_name: str, bitchains: List) -> float:
        ...
        """
        if dimension_name not in EntropyConfig.FractalStat_DIMENSIONS:
            return 0.0
    ...
    def compute_relative_contribution(self, dimension_name: str, bitchains: List, baseline_entropy: float) -> float:
        ...
        other_dims = [d for d in EntropyConfig.FractalStat_DIMENSIONS if d != dimension_name]
        ...
        try:
            ...
            return 100.0 / len(EntropyConfig.FractalStat_DIMENSIONS)
    ...
    def compute_expressiveness_contribution(
        ...
        composite = (
            EntropyConfig.EXPRESSIVENESS_WEIGHTS['individual'] * individual +
            EntropyConfig.EXPRESSIVENESS_WEIGHTS['relative'] * relative +
            EntropyConfig.EXPRESSIVENESS_WEIGHTS['complementary'] * complementary +
            EntropyConfig.EXPRESSIVENESS_WEIGHTS['ablation'] * ablation_contrib
        )
    ...
    def run(self) -> Tuple[List[EXP03_Result], bool]:
        ...
        baseline_coords = self.extract_coordinates(bitchains, EntropyConfig.FractalStat_DIMENSIONS)
        ...
        result = EXP03_Result(
            dimensions_used=EntropyConfig.FractalStat_DIMENSIONS.copy(),
        ...
        for i, removed_dim in enumerate(EntropyConfig.FractalStat_DIMENSIONS, 1):
            print("=" * 70)
            ...
            remaining_dims = [d for d in EntropyConfig.FractalStat_DIMENSIONS if d != removed_dim]
            ...
            result = EXP03_Result(
                ...
                meets_threshold=composite_score >= EntropyConfig.CRITICALITY_THRESHOLDS['expressiveness'],  # Use composite score for threshold
            )
            self.results.append(result)
        ...
        for i, result in enumerate(ranked, 1):
            removed_dim = [
                d for d in EntropyConfig.FractalStat_DIMENSIONS if d not in result.dimensions_used
            ][0]
    ...
    def generate_visualization_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_entropy": self.baseline_entropy,
            "threshold": EntropyConfig.CRITICALITY_THRESHOLDS['expressiveness'],
        }
...
def run_experiment_from_config(config: Optional[Dict[str, Any]] = None) -> Tuple[List[EXP03_Result], bool, Dict[str, Any]]:
    """
