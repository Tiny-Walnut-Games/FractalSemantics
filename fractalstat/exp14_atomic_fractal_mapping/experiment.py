"""
EXP-14: Atomic Fractal Mapping - Experiment Logic

This module contains the core experiment logic for the atomic fractal mapping test.
It implements the shell-based approach that maps electron shell configurations to
fractal parameters and validates whether atomic structure naturally emerges from
fractal hierarchy.
...
class AtomicFractalMappingExperiment:
    """
    ...
    def __init__(self, elements_to_test: List[str] = None):
        """
        Initialize atomic fractal mapping experiment.

        Args:
            elements_to_test: List of elements to test (if None, uses all available)
        """
        self.shell_data = get_electron_shell_data()
        
        if elements_to_test is None:
            self.elements_to_test = list(self.shell_data.keys())
        else:
            # Validate that all requested elements exist in our data
            missing_elements = [e for e in elements_to_test if e not in self.shell_data]
            if missing_elements:
                raise ValueError(f"Missing shell data for elements: {missing_elements}")
            self.elements_to_test = elements_to_test

        self.mappings: Dict[str, ShellBasedFractalMapping] = {}
        self.density_errors: List[float] = []
    ...
    def create_shell_based_fractal_mapping(self, element: str) -> ShellBasedFractalMapping:
        """
        Create a shell-based mapping from electron configuration to fractal structure.

        This is the CORRECTED approach: Use actual atomic structure as input.
        """
        if element not in self.shell_data:
            raise ValueError(f"No shell data for element: {element}")

        electron_config = self.shell_data[element]
    ...
    def run(self) -> Dict[str, Any]:
        """
        Run the complete atomic fractal mapping experiment.

        Returns:
            Complete experiment results with structure validation
        """
        print("\n" + "=" * 80)
        ...
        for element in self.elements_to_test:
            try:
                mapping = self.create_shell_based_fractal_mapping(element)
                self.mappings[element] = mapping
                self.density_errors.append(mapping.density_error)

                # Collect structure validation data
                self.structure_validation["depth_matches"].append(mapping.depth_matches_shells)
                self.structure_validation["branching_matches"].append(mapping.branching_matches_valence)
                self.structure_validation["exponential_growth"].append(mapping.node_growth_exponential)
        ...
        depth_accuracy = sum(self.structure_validation["depth_matches"]) / len(self.structure_validation["depth_matches"]) if self.structure_validation["depth_matches"] else 0
        branching_accuracy = sum(self.structure_validation["branching_matches"]) / len(self.structure_validation["branching_matches"]) if self.structure_validation["branching_matches"] else 0
        exponential_consistency = sum(self.structure_validation["exponential_growth"]) / len(self.structure_validation["exponential_growth"]) if self.structure_validation["exponential_growth"] else 0

        # Calculate density prediction statistics
        if self.density_errors:
            mean_error = statistics.mean(self.density_errors)
            std_error = statistics.stdev(self.density_errors) if len(self.density_errors) > 1 else 0
            max_error = max(self.density_errors)
            min_error = min(self.density_errors)

            # Correlation analysis
            predicted_densities = [m.predicted_density for m in self.mappings.values()]
            actual_densities = [m.actual_density for m in self.mappings.values()]
            ...
            print(f"Depth matches shell count:     {depth_accuracy:.1%} ({sum(self.structure_validation['depth_matches'])}/{len(self.structure_validation['depth_matches'])})")
            print(f"Branching matches valence:     {branching_accuracy:.1%} ({sum(self.structure_validation['branching_matches'])}/{len(self.structure_validation['branching_matches'])})")
            print(f"Exponential node growth:       {exponential_consistency:.1%} ({sum(self.structure_validation['exponential_growth'])}/{len(self.structure_validation['exponential_growth'])})")
        ...
        results = {
            ...
            "mappings": {
                ...
                for element, mapping in self.mappings.items()
            },
            ...
            "density_statistics": {
                "mean_density_error": round(mean_error, 4) if self.density_errors else None,
                "std_density_error": round(std_error, 4) if self.density_errors else None,
                "max_density_error": round(max_error, 4) if self.density_errors else None,
                "min_density_error": round(min_error, 4) if self.density_errors else None,
                "prediction_correlation": round(correlation, 4),
                "elements_mapped": len(self.mappings)
            },
            "success_criteria": {
                "structure_success": structure_success,
...
def main():
    ...
    try:
        ...
        status = "PASSED" if test_results["success_criteria"]["passed"] else "FAILED"
        print(f"Status: {status}")
        print(f"Output: {output_file}")

        if test_results["success_criteria"]["passed"]:
            ...
            print(f"   Depth accuracy: {test_results['structure_validation']['depth_accuracy']:.1%}")
            print(f"   Branching accuracy: {test_results['structure_validation']['branching_accuracy']:.1%}")
